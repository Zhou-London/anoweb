...
func setupTrackingRepo(t *testing.T) *FanTrackingRepository {
	...
	if err := db.AutoMigrate(&FanTracking{}); err != nil {
...
func forceTimestamps(repo *FanTrackingRepository, tracking *FanTracking) {
	repo.db.Exec(
...
func TestStartTrackingSkipsGuests(t *testing.T) {
	...
	var count int64
	if err := repo.db.Model(&FanTracking{}).Count(&count).Error; err != nil {
...
func TestStartTrackingFinalizesExistingSessions(t *testing.T) {
	...
	if err := repo.db.Model(&FanTracking{}).
