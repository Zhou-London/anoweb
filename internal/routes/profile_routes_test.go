...
func TestProfileCRUD(t *testing.T) {
	...
	if err := json.Unmarshal(getResp.Body.Bytes(), &createdProfile); err != nil {
		t.Fatalf("failed to decode profile: %v", err)
	}

	if createdProfile.Name != "Tester" || createdProfile.Email != "tester@example.com" {
		t.Fatalf("unexpected profile data: %+v", createdProfile)
	}
